...
Lane definitions (from UC-02 spec §8.4):
- reasoning  → Claude Opus  (expensive, best quality)
- middle     → Claude Sonnet (balanced, default for UC-02)
- triage     → Claude Haiku  (fast, cheap)
- air_gap    → Llama-3.3 70B via vLLM (no external data egress)

The default provider for all lanes is Anthropic.  Agent definitions can
override the provider via ``llm.provider`` or individual models via
``llm.model_overrides``.  See ``resolve_model()`` for the full resolution order.
"""
